"""
tools/backtest_2year.py

Massive 2-Year Multi-Leverage Backtester across ALL 24 coins without exclusions.
Handles uneven token genesis dates (e.g. WIF, ONDO) by aligning all data to BTC's
parent timeline. Trains HMM every 14 days to optimize the 1.6-million candle compute.
Outputs directly to 2year_matrix_results.txt.
"""
...
# The absolute full, un-pruned list
SEGMENT_COINS = config.CRYPTO_SEGMENTS

FETCH_DAYS = "180 days ago UTC"
INTERVAL = "15m"
BARS_PER_DAY = 96
TRAIN_BARS = 30 * BARS_PER_DAY
STEP_BARS = 14 * BARS_PER_DAY # Train every 14 days

LEVERAGES = [1, 5, 10, 15, 20, 35, 50, 100]
MARGIN_PER_TRADE = 100.0
FEE = 0.0005 # EXACT 0.05% commission per leg
...
class Trade:
    ...
    def calculate_pnl(self, leverage):
        ...
        return max(net, -MARGIN_PER_TRADE) 

def train_hmm(df, coin):
    features = get_features_for_coin(coin)
...
def run_simulation():
    # Write everything to a file to prevent terminal buffer pipe hanging
    orig_stdout = sys.stdout
    f_out = open('tools/6month_matrix_results.txt', 'w')
    sys.stdout = f_out
    
    print("="*80)
    print("🚀 INITIALIZING 6-MONTH OPTIMIZED MATRIX BACKTEST (ALL 24 COINS | $100 MARGIN | 0.05% FEE)")
    print("="*80)
    ...
    if "BTCUSDT" not in raw_data:
        print("Fatal: BTCUSDT missing. Cannot build master timeline.")
        return
        
    master_df = raw_data["BTCUSDT"][['timestamp']].drop_duplicates().reset_index(drop=True)
    total_bars = len(master_df)
    
    # Align all coins to BTC timeline so we don't truncate history for new coins (like WIF)
    data = {}
    for coin, df in raw_data.items():
        aligned = pd.merge(master_df, df.drop_duplicates(subset=['timestamp']), on='timestamp', how='left')
        data[coin] = aligned
        
    print(f"\n✅ Data aligned to Master Timeline: {total_bars} candles (~{total_bars//96} days).")
    # f_out.flush()
    ...
    for block_start in range(TRAIN_BARS, total_bars, STEP_BARS):
        ...
        print(f"🔄 Processing block {block_start} -> {block_end} ({(block_start/total_bars)*100:.1f}%)")
        # f_out.flush()
        ...
        for seg, coins in SEGMENT_COINS.items():
            for coin in coins:
                if coin in data:
                    train_df = data[coin].iloc[block_start - TRAIN_BARS : block_start].dropna()
                    if len(train_df) > TRAIN_BARS * 0.5: # At least 50% data available for training
                        # train_hmm inherently uses get_features_for_coin(coin)
                        mod, meta = train_hmm(train_df, coin)
                        if mod:
                            models[coin] = (mod, meta)
        ...
        for i in range(block_start, block_end):
            ...
            for seg, coins in SEGMENT_COINS.items():
                ...
                if c in data and i > 96:
                    vol_slice = pd.to_numeric(data[c]['vol_zscore'].iloc[i-96:i], errors='coerce')
                    if vol_slice.notna().sum() > 48:
                        heats[seg] = vol_slice.mean()
                    else: heats[seg] = -999
                else: heats[seg] = -999
            ...
            for seg in active_segs:
                for coin in SEGMENT_COINS[seg]:
                    if coin not in data or coin not in models: continue
                    bar = data[coin].iloc[i]
                    
                    if pd.isna(bar['close']): continue # Token may not exist yet in timeline
                    
                    mod, (scaler, state_map, features) = models[coin]
                    if any(pd.isna(bar[f]) for f in features): continue
    ...
    print(f"Total Unique Evaluation Nodes (Trades Evaluated Matrix): {len(trades)}")
    ...
    print("\n📊 2-YEAR LEVERAGE PERFORMANCE MATRIX")
    ...
    for (seg, rm_id), st_trades in group_trades.items():
        ...
        print(f"\n--- {seg} | {rm_id} ---")
        print(f"{'Lev':<5} | {'Trades':<6} | {'Net PnL ($)':<12} | {'Max DD ($)':<11} | {'Win%':<6} | {'PF':<6} | {'R/R':<6} | {'Sharpe':<6}")
        
        st_trades.sort(key=lambda x: getattr(x, 'exit_idx', 0))
        ...
        for lev in LEVERAGES:
            ...
            print(f"{lev:<4}x | {len(st_trades):<6} | ${cum_pnl:<11.2f} | ${max_dd:<10.2f} | {win_rate:>5.1f}% | {pf:>4.2f} | {rr:>4.2f} | {sharpe:>5.2f}")
