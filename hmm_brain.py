...
class HMMBrain:
    ...
    def __init__(self, n_states=None, symbol=None, features_list=None):
        ...
        self._pca = None              # PCA object for covariance stability
    ...
    def train(self, df):
        ...
        # ── PCA Quant Filter ──────────────────────────────────────────────────────────
        # Fixes structural 'Covariance Collapse' generated by highly colinear variables
        # by extracting independent orthogonal components. Retain 98% of variance.
        try:
            from sklearn.decomposition import PCA
            # We want up to N_COMPONENTS but limited by the number of raw features
            n_comp = min(features_fit.shape[1], 5)
            self._pca = PCA(n_components=n_comp, whiten=True, random_state=42)
            features_fit = self._pca.fit_transform(features_fit)
        except Exception as e:
            logger.warning("PCA Transform failed for %s: %s", self.symbol, e)
            self._pca = None
    ...
    def _build_state_map(self):
        ...
        if raw_means.ndim == 3:
            ...
            # If PCA is active, inverse transform state_means before indexing
            if self._pca is not None:
                state_means = self._pca.inverse_transform(state_means)
    ...
    def predict(self, df):
        ...
        # Apply PCA projection if it was initialized during training
        if self._pca is not None:
            try:
                features_scaled = self._pca.transform(features_scaled)
            except Exception as e:
                logger.warning("PCA prediction transform failed for %s", self.symbol)
                return config.REGIME_CHOP, 0.0
